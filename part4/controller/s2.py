...
def scheduler(memcached, workloads, sys_cpu_util):
    ...
    if main_workload is None:
        if len(completed) == 0:
            ...
            workloads['blackscholes'].start(num_threads=4, core_list=[0, 3])
    ...
    if 'canneal' in serials:
        ...
        if workloads['canneal'].state == Workload.RUNNING and cpu_util is not None and cpu_util > 1:
            if main_workload != 'blackscholes':
                print('!!!', main_workload)
            old_cores = workloads[main_workload].core_list
            if 2 not in old_cores:
                workloads[main_workload].adjust_cores(old_cores + [2])
...
def main():
    ctrl = controller.Controller(scheduler, log_name='s2', memcached_core_list=[0, 1, 2, 3])
    ctrl.run()
