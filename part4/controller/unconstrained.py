...
def scheduler(memcached, workloads, sys_cpu_util):
    ...
    if num_completed == 0:
        workloads['fft'].start(num_threads=4, core_list=[0, 1, 2, 3])
        ...
        workloads['canneal'].start(num_threads=4, core_list=[0, 1, 2, 3])
...
def main():
    ctrl = controller.Controller(scheduler, log_name='unconstrained', memcached_core_list=[0, 1, 2, 3])
    ctrl.run()
